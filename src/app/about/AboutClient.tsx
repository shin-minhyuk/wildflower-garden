    ...
    <div className="w-full max-w-4xl mx-auto px-6 py-16 md:py-24 space-y-20 font-gowun-batang animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          ...
          <div
            key={idx}
            className="flex flex-col items-center text-center p-6 bg-[#FAF8F5] rounded-xl hover:shadow-md transition-shadow"
          >
            <div className="w-12 h-12 flex items-center justify-center rounded-full bg-white text-primary shadow-sm mb-4">
              <item.icon className="w-6 h-6" />
            </div>
            <h4 className="text-lg font-bold text-[#2E5C3C] mb-2">
              {item.label}
            </h4>
        ...
        {historyData.map((item, index) => (
          ...
          >
            ...
            <div className="flex-1 space-y-4 text-center md:text-left">
              ...
              <h3 className="text-2xl md:text-3xl font-bold text-[#2E5C3C]">
                {item.title}
              </h3>
...
export default function AboutClient() {
  const [activeTab, setActiveTab] = useState<"philosophy" | "history">(
    "philosophy"
  );
